...
export const tsCloud: TsCloudConfig = {
  ...
  environments: {
    production: {
      type: 'production',
      // Push to `main` → deploy here (bare apex loghq.org).
      deployBranch: 'main',
      region: 'us-east-1',
      variables: {
        NODE_ENV: 'production',
    ...
    staging: {
      type: 'staging',
      // Push to `stage` → deploy here, served at `staging.loghq.org` on the
    ...
    development: {
      type: 'development',
      // Push to `dev` → deploy here, served at `dev.loghq.org`.
      deployBranch: 'dev',
      domainPrefix: 'dev',
      region: 'us-east-1',
      variables: {
  ...
  infrastructure: {
    ...
    ssl: {
      ...
      domains: env.SSL_DOMAINS?.split(',') || ['loghq.org', 'www.loghq.org'],
      redirectHttp: true,
      // Porkbun manages DNS for loghq.org. ts-cloud auto-detects the provider
     ...
     */
    dns: {
      // loghq.org is registered + DNS-managed at Porkbun (provider auto-detected
  ...
  sites: {
    ...
    // `loghq-main` service, so the generic `main` key can never collide with
    ...
    // www → apex redirect.
    www: { domain: `www.${env.APP_DOMAIN || 'loghq.org'}`, redirect: `https://${env.APP_DOMAIN || 'loghq.org'}` },
  },
}
