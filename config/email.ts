...
export default {
  ...
  // MAIL_FROM_ADDRESS) to any provisioned mailbox — e.g. hello@loghq.org — and,
  // as long as that mailbox's MAIL_PASSWORD_<LOCALPART> is set, the SMTP driver
  // authenticates as it automatically.
  from: {
    name: env.MAIL_FROM_NAME || 'LogHQ',
    address: env.MAIL_FROM_ADDRESS || `noreply@${env.MAIL_DOMAIN || 'loghq.org'}`,
  },

  domain: env.MAIL_DOMAIN || 'loghq.org',
