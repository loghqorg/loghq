...
export async function sendInviteEmail(email: string, projectName: string, token: string, inviterName?: string): Promise<void> {
  ...
  const subject = `You're invited to ${projectName} on loghq`
  const text = [
    `${who} to collaborate on ${projectName} on loghq.`,
    '',
    `Join here: ${url}`,
    '',
    `If you don't have a loghq account yet, you'll be able to create one — just use this email address (${email}).`,
  ].join('\n')
...
<div style="font-family:ui-sans-serif,system-ui,sans-serif;max-width:520px;margin:0 auto;padding:24px 16px;color:#0b0f19">
  ...
  <p style="margin:20px 0 0;font-size:12px;color:#97a1b2">If you don't have a loghq account yet, you'll create one on the next screen — sign up with <span style="color:#4b5565">${escapeHtml(email)}</span>. If you weren't expecting this, you can ignore this email.</p>
</div>`
