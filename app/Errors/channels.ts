...
// could store an arbitrary URL would turn loghq into a request proxy into our
...
// loghq rose; regressions use amber so a re-break reads differently at a glance.
...
function slackPayload(projectName: string, issue: ChannelIssue, kind: AlertKind): unknown {
  ...
  return {
    ...
    text: `[loghq] ${heading(kind)} in ${projectName}: ${issue.title}`,
    blocks: [
      ...
      { type: 'actions', elements: [{ type: 'button', text: { type: 'plain_text', text: 'Open in loghq', emoji: true }, url, style: 'danger' }] },
    ],
  }
}
...
export async function sendTestAlert(type: ChannelType, webhookUrl: string, projectName: string): Promise<boolean> {
  const sample: ChannelIssue = {
    id: 'test',
    title: 'loghq test alert — your integration works 🎉',
