...
function memRateLimit(key: string, limit: number, windowMs: number, count = 1): { ok: boolean, retryAfter: number } {
  const nowMs = Date.now()
  const w = windows.get(key)
  if (!w || nowMs >= w.resetAt) {
    windows.set(key, { count, resetAt: nowMs + windowMs })
...
async function redisRateLimit(key: string, limit: number, windowMs: number, count = 1): Promise<{ ok: boolean, retryAfter: number } | null> {
  if (!client || Date.now() < redisDownUntil)
    return null
  const cacheKey = `ratelimit:${key}`
  const windowSec = Math.max(1, Math.ceil(windowMs / 1000))
  try {
    const n = Number(count === 1 ? await client.incr(cacheKey) : await client.send('INCRBY', [cacheKey, String(count)]))
    if (n <= count)
 ...
 */
export async function rateLimit(key: string, limit: number, windowMs: number, count = 1): Promise<{ ok: boolean, retryAfter: number }> {
  const viaRedis = await redisRateLimit(key, limit, windowMs, count)
  return viaRedis ?? memRateLimit(key, limit, windowMs, count)
}
